  ...
  const handleTanggalChange = (event) => {
    ...
    if (selectedDateObject >= minAllowedDate) {
      ...
      Swal.fire({
        ...
        text: `${TypeEvent === "one_time" ? "Tanggal tidak boleh kurang dari hari ini." : "Pilihlah tanggal minimal 2 hari dari tanggal saat ini."}`,
  ...
  return (
    <>
      ...
      <div className="p-2 mt-2 w-full px-5 space-y-3">
        ...
        <div className="flex flex-row gap-4 h-14 items-center">
          {onFocusTime && (
          ...
          <div className="flex h-14 flex-row items-center p-4 py-0 bg-gray-100 text-gray-400 text-sm rounded-lg focus:ring-blue-500 w-full focus:border-none my-2">
            <IconClock />
            <button
...
function StepThree({
  ...
  const handleQuantityChange = (e, IdMerchan, itemId, capacity) => {
    const value = parseInt(e.target.value); // Ambil nilai dari input
    ...
    if (itemIndex !== -1) {
      const newQuantity = Math.min(Math.max(value, 1), capacity); // Batasi nilai antara 1 dan capacity
      updatedCart[itemIndex].quantity = newQuantity;
      updatedCart[itemIndex].total =
        updatedCart[itemIndex].quantity * updatedCart[itemIndex].price;

      // Hitung ulang total harga dan jumlah item dalam cart
  ...
  const handleSubmit = async () => {
    ...
    const products = cart.map((item) => {
      if (isNaN(item.quantity) || item.quantity < 1) {
        Swal.fire({
          ...
          text: "Produk dengan kuantitas tidak valid ditemukan. Pastikan semua produk memiliki kuantitas lebih dari 1.",
          showConfirmButton: false,
          timer: 2000,
        });
        setLoading(false);
    ...
    const invalidProducts = cart.some(
    ...
    if (campData?.TypeEvent === "one_time") {
                          ...
                          <div className="grid place-items-center">
                            <div className="flex items-center">
                              ...
                              <input
                                ...
                                min={1}
                                max={item.capacity}
                                className="w-12 px-2 text-center text-blue-700 font-bold border rounded-md border-blue-900"
                                onChange={(e) =>
...
function Stepfive({
  ...
  useEffect(() => {
    if (!loading && dataApi.length === 0) {
      if (detonatorId && token && coordinates.lat && coordinates.lng) {
        setLoading(true);
        fetchData();
      }
    }
  }, [detonatorId, token, coordinates.lat, coordinates.lng]); // Removed setLoading(false) from else block
  ...
  const fetchData = async () => {
    try {
      ...
      // Filter by distance and not NaN
      ...
      const merchantsWithinRange = validMerchants.filter(
        (merchant) => merchant.distance <= 25000
      );

      if (merchantsWithinRange.length === 0) {
        Swal.fire({
          icon: "error",
          title: "Oops...",
          text: "Tidak ada merchant terdekat di lokasi Anda. Silakan pilih lokasi yang lain",
        ...
        const sortedMerchants = merchantsWithinRange.sort(
      ...
      setLoading(false); // Ensuring loading is set to false after the API call
    }
  };
