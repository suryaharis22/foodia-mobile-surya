...
const DetailCamp = () => {
  ...
  const [merchantReportLength, setMerchantReportLength] = useState(0);
  const [detonatorReportLength, setDetonatorReportLength] = useState(0);
  const { state, setDonation } = useAppState();
  const [nominalDonasi, setNominalDonasi] = useState(0);

  // Function to fetch report dynamically based on type
  const fetchReport = async (type, setReportLength) => {
    try {
      ...
      setReportLength(response.data.body.length);
    } catch (error) {
      setLoading(false);
  ...
  useEffect(() => {
    if (idCamp) {
      setLoading(true);
      fetchReport("merchant", setMerchantReportLength);
      fetchReport("detonator", setDetonatorReportLength);
    }
  }, [idCamp]);
